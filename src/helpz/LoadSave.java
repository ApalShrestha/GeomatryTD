...
public class LoadSave {
    ...
    public static BufferedImage getSpriteAtlas() {
        try {
            // Direct file access - your res folder is in project root
            File file = new File("res/spriteatlas.png");
            System.out.println("Loading sprite from: " + file.getAbsolutePath());
            
            if (file.exists()) {
                BufferedImage img = ImageIO.read(file);
                System.out.println("SUCCESS: Sprite loaded - " + img.getWidth() + "x" + img.getHeight());
                return img;
            } else {
                System.err.println("ERROR: spriteatlas.png not found at: " + file.getAbsolutePath());
            }
        } catch (IOException e) {
            System.err.println("ERROR: Failed to load sprite atlas!");
            e.printStackTrace();
        }
        return null;
    }

    // The rest of your methods can stay the same
